...
A PHP client SDK for the Insignia Education API (`/api/v1`), mirroring the structure and
conventions of [`insignia-education/api-sdk-js`](../api-sdk-js) — the mature reference
implementation for this API:

- One class per API resource (`Courses`, `Users`, `Auth`, …), matching HTTP verbs
...
- Minimal runtime dependencies (Guzzle or PHP's cURL extension are reasonable — JS's
  zero-dependency constraint doesn't map 1:1 to PHP).
- No hardcoded human-readable strings — thrown exceptions expose a machine-readable status/body;
  translation is the consuming app's job.
...
Check whether this SDK is meant for a PHP consumer of the API (e.g. server-to-server calls from
`crm-api` or another Laravel backend) before assuming its shape should mirror a browser-facing
client — a server-side PHP SDK may reasonably support things `api-sdk-js` intentionally avoids
(e.g. persistent connections, retry/backoff). Confirm the intended consumer with the repo owner
before designing the client shape.
